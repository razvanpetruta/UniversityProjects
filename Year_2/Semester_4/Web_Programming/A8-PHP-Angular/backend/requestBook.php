<?php
    ...
    if ($rentalCount > 0) {
        $response = array('message' => 'Error: This book is already rented');
    ...
    if (mysqli_stmt_execute($stmt)) {
        $response = array('message' => "Book with ID $bookID has been requested for renting.");
        echo json_encode($response);
    } else {
        $response = array('error' => "Error: requesting the book: " . mysqli_error($conn));
        echo json_encode($response);
    }
