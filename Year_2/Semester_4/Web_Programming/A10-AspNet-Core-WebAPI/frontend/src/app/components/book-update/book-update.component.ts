...
export class BookUpdateComponent {
    ...
    this.activatedRoute.params.subscribe(params => {
      ...
      this.generalService.getBookByID(this.bookID!).subscribe({
        next: (response) => {
          ...
          this.author = response.value.author;
          this.noPages = response.value.noPages;
          this.price = response.value.price;
          this.genre = response.value.genre;
        }
      });
    });
  }
  ...
  onSubmit(form: any) {
    if (this.username && this.role && this.bookID && this.title && this.author && this.noPages && this.price && this.genre) {
      ...
      this.generalService.updateBook(this.bookID, this.title, this.author, this.noPages, this.price, this.genre).subscribe({
        next: (response) => {
          this.toastrService.success("Successfully updated", "", { progressBar: true });
        },
        error: (error) => {
          console.log(error);
        },
