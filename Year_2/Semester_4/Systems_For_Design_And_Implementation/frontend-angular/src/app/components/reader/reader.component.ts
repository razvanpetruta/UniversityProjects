...
export class ReaderComponent implements OnInit, OnDestroy {
  ...
  listReaders(): void {
    ...
    this.subscriptions.add(this.readerService.getPageReaders(this.pageNumber, this.pageSize).subscribe({
  ...
  onPageChanged(event: PageEvent) {
    ...
    this.subscriptions.add(this.readerService.countReaders().subscribe((result: Number) => {
      this.noPages = Math.floor(result.valueOf() / this.pageSize);
      if (result.valueOf() % this.pageSize > 0) {
        this.noPages++;
      }
