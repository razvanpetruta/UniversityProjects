...
Vector* makeCopy(Vector* v)
{
	...
	for (i = 0; i < v->size; i++)
	{
		Estate* e = createEstate(v->data[i]->type, v->data[i]->address, v->data[i]->price, v->data[i]->surface);
		if (e == NULL)
		{
...
void sortVector(Vector* result, int (*comparator)(Estate*, Estate*))
{
	...
	for (i = 0; i < result->size - 1; i++)
	{
		...
		{
			if ((*comparator)(result->data[i], result->data[j]) == 0)
			{
				Estate* aux = result->data[i];
...
void testAddToVector()
{
	...
	Estate* e = createEstate("house", "100", 10, 10);
	addToVector(v, e);
