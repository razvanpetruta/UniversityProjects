...
class RentalRepository:
    ...
    def find_book_in_rentals(self, book_id):
        ...
        """
        def custom_filter(rental):
            return rental.book_id == book_id and rental.returned_date is None

        sol = GenericFilter(self.list_of_rentals, custom_filter).filter()
        if len(sol) != 0:
            return sol[0]
        return None
    ...
    def rent_date_greater_than_return_date(self, book_id, rented_date):
        ...
        """
        def custom_filter(rental):
            return rental.book_id == book_id and rental.returned_date > rented_date
    ...
    def find_book_with_client(self, book_id, client_id):
        ...
        """
        def custom_filter(rental):
            return rental.book_id == book_id and rental.client_id == client_id and rental.returned_date is None
    ...
    def most_rented_books(self):
        ...
        """
        book_rentals = dict()
        for key in self._rentals:
            book_id = self._rentals[key].book_id
    ...
    def most_active_clients(self):
        ...
        client_days = dict()
        for key in self._rentals:
            client_id = self._rentals[key].client_id
            days = 0
            if self._rentals[key].returned_date is None:
                days += (datetime.date.today() - self._rentals[key].rented_date).days
            else:
                days += (self._rentals[key].returned_date - self._rentals[key].rented_date).days
    ...
    def list_of_rentals(self):
        rentals_list = []
        for key in self._rentals:
            rentals_list.append(self._rentals[key])
        return rentals_list
