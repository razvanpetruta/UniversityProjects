...
class RentalService:
    ...
    def statistics_books(self):
        ...
        for key in self.__book_repo.books:
            if key not in books:
                books[key] = 0
        sol_sorted = sorted(books.items(), key=lambda x: x[1], reverse=True)
        return sol_sorted
    ...
    def statistics_authors(self):
        ...
        for _id in books:
            ...
            if author not in authors:
                ...
                authors[author] += books[_id]
        sol_sorted = sorted(authors.items(), key=lambda x: x[1], reverse=True)
        return sol_sorted
    ...
    def statistics_clients(self):
        ...
        for key in self.__client_repo.clients:
            if key not in clients:
                clients[key] = 0
        sol_sorted = sorted(clients.items(), key=lambda x: x[1], reverse=True)
        return sol_sorted
