using ParsingFinal.Implementation;

namespace ParsingFinal.Utils;

public class Menu
{
    private string ReadSeqFile()
    {
        string line = File.ReadAllText("Resources/seq.txt");
        ...
        while (true)
        {
            try
            {
                ...
                switch (option)
                {
                    ...
                    case 11:
                        var sequence = ReadSeqFile();
                        Console.WriteLine($"The sequence to be parsed: {sequence}");
