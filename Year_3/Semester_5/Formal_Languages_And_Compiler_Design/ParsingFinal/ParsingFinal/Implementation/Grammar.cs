...
public class Grammar
{
    ...
    public void PrintProductionLHSsForANonterminal(string nonterminal)
    {
        foreach (var production in GetProductionLHSsForANonterminal(nonterminal))
        {
            Console.WriteLine($"{production.LeftHandSide} -> {production.RightHandSide}");
        }
    }

    public List<Production> GetProductionLHSsForANonterminal(string nonterminal)
    {
    ...
    public void PrintProductionRHSsForANonterminal(string nonterminal)
    {
        foreach (var production in GetProductionRHSsForANonterminal(nonterminal))
        {
            Console.WriteLine($"{production.LeftHandSide} -> {production.RightHandSide}");
        }
    }
    
    public List<Production> GetProductionRHSsForANonterminal(string nonterminal)
    {
        var result = new List<Production>();

        foreach (var production in Productions)
        {
            if (production.RightHandSide.Split(" ").Contains(nonterminal))
            {
                result.Add(production);
            }
        }
